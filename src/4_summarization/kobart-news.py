...
def input_csv():
    return pd.read_csv('data/raw_data/rss_part3.csv', sep=';', keep_default_na=False)
...
def output_csv(df):
    df.to_csv('data/raw_data/rss_part4.csv', sep=';', index=False)


from tqdm import tqdm

if __name__ == '__main__':
    print("Step 4: Extract Summary from article body... ")
    ...
    with tqdm(total=len(df), unit="tasks", bar_format="{percentage:3.0f}% {bar} {n_fmt}/{total_fmt} [{elapsed}]") as progress_bar:
        for idx, row in df.iterrows():
            ...
            if doc != '':
                try:
                    summary_text_ids = summarize(doc)
