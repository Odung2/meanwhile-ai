...
model_en = SentenceTransformer('distilbert-base-nli-mean-tokens')
...
if __name__ == '__main__':
    ...
    with tqdm(total=len(df), unit="tasks", bar_format="{percentage:3.0f}% {bar} {n_fmt}/{total_fmt} [{elapsed}]") as progress_bar:
        for _, row in df.iterrows():
            ...
            if doc != '':
                if lang == 0:
                    candidates = tokenize_ko(doc)
                    # print(candidates)

                    doc_embedding = model_ko.encode([doc])
                    candidate_embeddings = model_ko.encode(candidates)
                else:
                    candidates = tokenize_en(doc)
                    # print(candidates)

                    doc_embedding = model_en.encode([doc])
